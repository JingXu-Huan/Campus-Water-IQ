...
你需要在阿里云控制台，请求一个密钥对。
然后在`AuthServiceApplication`设置环境变量。
例如：
![img_4.png](img_4.png)

### 2.GLM质谱大模型：

同样在控制台申请密钥，在`PredictionService`设置环境变量。
例如：
![img_5.png](img_5.png)
...
5. 在 `IoT-service/service/src/main/resources/application.yml` 中更新 token

> **注意这两个配置文件都要改。接入层和数据服务层都需要使用时序数据库。**
...
**1. 创建网桥，用于 Prometheus 和 Grafana 的容器内通信**
...
将 `IoT-device/prometheus.yml` 里的 IP 地址修改为你自己的本机 IP：

```yaml
static_configs:
  - targets: [ '你的IP:18097' ]  # 例如：192.168.1.100:18097
```
...
```bash
cd /home/jingxu/IdeaProjects/Campus-Water-IQ/frontend
```
