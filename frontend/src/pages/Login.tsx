  ...
  const handleSocialLogin = async (provider: 'GITHUB' | 'WECHAT') => {
    try {
      const res = provider === 'GITHUB' 
        ? await authApi.getGitHubAuthUrl() as unknown as string
        : await authApi.getWeChatAuthUrl() as unknown as string
      if (res) window.location.href = res
    } catch (err) {
      setError(`${provider === 'GITHUB' ? 'GitHub' : '微信'}登录失败，请稍后重试`)
    }
  }
            ...
            <div className="mt-8 pt-6 border-t border-gray-100">
              ...
              <div className="flex justify-center gap-3">
                ...
                >
                  ...
                  <span className="text-sm font-medium text-gray-600 group-hover:text-gray-900">微信</span>
                </button>
              </div>
            </div>
          </div>
